...
if [[ (-z $version) || ( $version != rc* && $version != dev* ) ]]
then
  echo "A version is required and should follow Semantic Versioning Major.Minor.Patch"
  echo "Dev and QA builds are required to start with 'dev' ex: dev1.0.1 or 'rc' ex: rc1.0.1"
fi
if [ -z "$text" ]
then
...
if [[ (-z "$version") || (-z "$text") || (-z "$hash") || ( $version != rc* && $version != dev* ) ]]
then
